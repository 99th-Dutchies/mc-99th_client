...
Source: "C:\sources\mc-99th_dutchclient\installers\mc-{#MyAppName}-{#MyAppVersion}.jar"; DestDir: "{app}"; DestName: "{#MCVersion}-{#MyAppName}.jar"; AfterInstall: AfterDuplicate; Flags: ignoreversion
...
begin
  ...
  { Add profile to launcher_profiles }
  ...
  { Add profile to launcher_profiles_microsoft_store }
  profilefilename := ExpandConstant('{app}\..\..\launcher_profiles_microsoft_store.json');
  LoadStringFromFile(profilefilename, profilecontent);
  profilecontentuni := String(profilecontent);
  profilepos := Pos('"lastVersionId" : "{#MCVersion}-{#MyAppName}"', profilecontentuni);
  if profilepos = 0 then
